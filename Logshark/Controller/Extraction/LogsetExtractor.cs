...
namespace Logshark.Controller.Extraction
{
    ...
    public class LogsetExtractor
    {
        ...
        public LogsetExtractor(LogsharkRequest request)
        {
        ...
        public void Process()
        {
            ...
            if (request.Target.IsDirectory || !PathHelper.ExistsOnLocalDrive(request.Target))
            {
                try
                {
                    request.Target.Target = CopyTargetLocally(request);
                }
        ...
        protected static string CopyTargetLocally(LogsharkRequest request)
        {
            ...
            ValidateSufficientDiskSpaceToCopyTargetLocally(request);
            ...
            if (request.Target.IsDirectory)
            {
                ...
                var requiredFiles = GetRequiredFilesInDirectory(request);
        ...
        private static void ValidateSufficientDiskSpaceToCopyTargetLocally(LogsharkRequest request)
        {
            ...
            if (request.Target.IsDirectory)
            {
                foreach (string requiredFile in GetRequiredFilesInDirectory(request))
                {
