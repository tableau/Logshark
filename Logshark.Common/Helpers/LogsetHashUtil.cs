...
namespace Logshark.Common.Helpers
{
    public class LogsetHashUtil
    {
        ...
        private static string ComputeZipFileHash(string targetPath)
        {
            ...
            {
                throw new Exception(fileExtension + " is not supported as an archive format!");
            }
            ...
            {
                throw new Exception(String.Format("Unable to access contents of archive '{0}': {1}", targetPath, ex.Message), ex);
            }
            finally
            {
                if (zipFile != null)
