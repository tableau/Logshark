...
using Logshark.Helpers.StatusWriter;
...
namespace Logshark.Controller.Parsing
{
    ...
    internal class MongoWriter
    {
        ...
        public MongoWriter(LogsharkRequest request)
        {
            logsharkRequest = request;
            logsetPreprocessor = new LogsetPreprocessor(request);
        ...
        private void CreateMongoDbCollections()
        {
            ...
            if (logsharkRequest.RunContext.LogsetType == LogsetType.Desktop)
            {
                parsers = ParserFactory.GetDesktopParsers();
            }
            else
            {
                parsers = ParserFactory.GetServerParsers();
            }
