...
- `Timeout` - generic timeout (in seconds) for requests to Tableau Server.
- `PublishingTimeout` - separate, usually higher, timeout (in seconds) that applies only to the publishing step, since Tableau Server can take a while to generate thumbnails for large workbooks.
- `ApplyPluginProvidedTagsToWorkbooks` - if `true` (default), applies the tags each plugin provides to its generated workbook on publish.
- `ProjectDescriptionFooterHtml` - additional text (HTML allowed) appended to the description of the project LogShark creates on Tableau Server. Can also be set with the `--ts-project-description-footer` command line option.
- `SkipPublishingEmptyWorkbooks` - if `true` (default), workbooks with no data in any of their extracts are not published to Tableau Server.
