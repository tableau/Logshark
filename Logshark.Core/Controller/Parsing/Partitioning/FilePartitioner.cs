...
namespace Logshark.Core.Controller.Parsing.Partitioning
{
    /// <summary>
    ...
    internal class FilePartitioner
    {
        ...
        public FilePartitioner(LogsharkRequest request, LogFileContext file, long partitionSizeBytes)
        {
        ...
        private LogFileContext WritePartition(IEnumerator<string> lineIterator, int partitionIndex)
        {
            ...
            using (var writer = File.CreateText(partitionName))
            {
                ...
                while (bytesWritten <= partitionSizeBytes)
                {
                    ...
                    if (!lineIterator.MoveNext())
                    {
                        ...
                        return new LogFileContext(partitionName, file.RootLogDirectory, file.FileName, lineOffset);
                    }
        ...
        private static string GeneratePartitionName(string sourceFile, int partitionIndex)
        {
